...
$action       = 'add';
$edit_mode    = 0;
if ( isset( $_GET['action'] ) && $_GET['action'] == 'edit_plan' && isset( $_GET['id'] ) && ! empty( $_GET['id'] ) ) { //phpcs:ignore
	$edit_mode = 1;
	$plan_id   = intval( $_GET['id'] ); //phpcs:ignore
	...
	$plan_options['expiry_date']       = ! empty( $plan_options['expiry_date'] ) ? $plan_options['expiry_date'] : date( 'Y-m-d 23:59:59' ); //phpcs:ignore
}
	...
	<div class="content_wrapper arm_subscription_plan_content" id="content_wrapper">
		...
		<form  method="post" id="arm_add_edit_plan_form" class="arm_add_edit_plan_form arm_admin_form">
			...
			<div class="arm_admin_form_content">
				...
				<div id="arm_plan_price_box_content" class="arm_plan_price_box">
					<div class="page_sub_content">
						...
						<table class="form-table">
							...
							<tr class="form-field paid_subscription_options_recurring_payment_cycles_main_box_tr <?php echo ( $subscription_type == 'recurring' ) ? '' : 'hidden_section'; ?>">
								...
								<td>
									...
									<div class="paid_subscription_options_recurring_payment_cycles_link">
											...
											<input type="hidden" name="arm_total_recurring_plan_cycles" id="arm_total_recurring_plan_cycles" value="<?php echo !empty( $total_inirecurring_cycle ) ? esc_attr($total_inirecurring_cycle) : 1; ?>"/>
                                            <input type="hidden" name="arm_total_recurring_plan_cycles_counter" id="arm_total_recurring_plan_cycles_counter" value="<?php echo !empty( $total_inirecurring_cycle ) ? esc_attr($total_inirecurring_cycle) : 1; //phpcs:ignore?>" />
										</div>
									</div>
								</td>
							</tr>
							...
							<tr class="form-field paid_subscription_end_of_term <?php echo ( $subscription_type == 'paid_finite' || $subscription_type == 'recurring' ) ? '' : 'hidden_section'; ?>" >
				    ...
				    ?>
							...
							<tr class="form-field paid_subscription_grace_period_end_of_term <?php echo ( $subscription_type == 'recurring' || $subscription_type == 'paid_finite' ) ? '' : 'hidden_section'; ?>">
							...
							<tr class="form-field paid_subscription_upgrade_downgrade <?php echo (($subscription_type == 'paid_infinite' || $subscription_type == 'paid_finite') || $subscription_type == 'recurring' ) ? '' : 'hidden_section'; ?>">
								<?php
				...
				if ( isset( $_GET['action'] ) && $_GET['action'] == 'edit_plan' && $totalPlanMembers > 0 ) { //phpcs:ignore
					?>
